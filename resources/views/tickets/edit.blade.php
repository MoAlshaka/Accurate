        ...
        <div class="col-lg-12 col-md-12">
            <div class="card">
                <div class="card-body">
                    ...
                    <form action="{{ route('tickets.update', ['ticket' => $ticket->id]) }}" data-parsley-validate=""
                        ...
                        <div class="row row-sm">
                            <div class="col-4 mg-lg-b-20">
                                <div class="form-group">
                                    <input class="form-control" name="code" placeholder="الكود" type="text"
                                        value="@if ($ticket->name) {{ $ticket->name }} @endif">
                                </div>
                                @error('code')
                            ...
                            <div class="parsley-select col-lg-4 col-md-8 mg-lg-b-20" id="slWrapper">
                                <select class="form-control select1" data-placeholder="العميل" required=""
                                    name="customer_id">
                                    <option value="{{ $ticket->customer->id }}">{{ $ticket->customer->name }}
                                    </option>
                                    @isset($customers)
                            ...
                            <div class="parsley-select col-lg-4 col-md-8 mg-lg-b-20" id="slWrapper">
                                <select class="form-control select1" data-placeholder="قسم الدعم" required=""
                                    ...
                                    <option value="{{ $ticket->support_cate->id }}">{{ $ticket->support_cate->name }}
                                    </option>
                                    @isset($supportCates)
                            ...
                            <div class="parsley-select col-lg-3 col-md-8 mg-lg-b-20" id="slWrapper">
                                <select class="form-control select1" data-placeholder="الحالة" required=""
                                    name="status">
                                    <option value="{{ $ticket->status }}">{{ $ticket->status }}
                                    </option>
                            ...
                            <div class="col-12 mg-lg-b-20">
                                <textarea class="form-control" placeholder="التفاصيل" rows="3" name="notes" required="">{{ $ticket->notes }}</textarea>
                                @error('notes')
                                    <div class="tx-danger">{{ $message }}</div>
                                @enderror
                            </div>
